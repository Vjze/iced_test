...
# Linux
APPIMAGE_NAME = Iced_Test.AppImage
APPIMAGE_DIR = $(RELEASE_DIR)/AppDir
APPIMAGE_DESKTOP_FILE = $(RESOURCES_DIR)/linux/Iced_Test.desktop
APPIMAGE_ICON_FILE = $(RESOURCES_DIR)/icons/256x256/logo.png

TAR_NAME = Iced_Test.tar.gz
...
ifdef OPENGL
  DMG_NAME :=Iced_Test-opengl.dmg
  APPIMAGE_NAME :=Iced_Test-opengl.AppImage
else
  DMG_NAME :=Iced_Test.dmg
  APPIMAGE_NAME :=Iced_Test.AppImage
  FEATURE_FLAG :=
endif
